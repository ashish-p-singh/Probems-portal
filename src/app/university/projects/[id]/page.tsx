          ...
          <div className="col-span-12 lg:col-span-8 space-y-4">
              ...
              <div className="card p-6">
                {!project.team ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-sm text-slate-500 mb-4">No team formed yet.</p>
                    <button
                      onClick={() => act('CREATE_TEAM', {
                        teamName: 'Project Team',
                        teamDescription: 'Multidisciplinary team for this project',
                      })}
                      disabled={submitting}
                  ...
                  <div className="space-y-4">
                    <div>
                      <h3 className="text-sm font-semibold text-slate-900">{project.team.name}</h3>
                      {project.team.description && <p className="text-xs text-slate-500 mt-0.5">{project.team.description}</p>}
                    </div>
                    {project.facultyAssignment && (
                      <div className="flex items-center gap-3 p-3 bg-violet-50 rounded-lg border border-violet-100">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-900">{project.facultyAssignment.faculty?.name}</p>
                          <p className="text-xs text-violet-600">Faculty Leader</p>
                        </div>
                      </div>
                    )}
                      ...
                      {project.team.members.map((m: any) => (
                        <div key={m.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                          <div className="w-8 h-8 bg-white border border-slate-200 rounded-full flex items-center justify-center">
                            <span className="text-sm font-medium text-slate-600">{m.user.name.charAt(0)}</span>
                          </div>
            ...
            {activeTab === 'solution' && (
              <div className="space-y-6">
                <div className="card p-6">
                  {!project.solution ? (
                    <p className="text-sm text-slate-500 text-center py-6">No solution submitted yet.</p>
                  ) : (
                    <div className="space-y-4">
